...
async function generatePage1(
  ...
  if (note.expose) {
    ...
    const exposeLines = doc.splitTextToSize(note.expose, pageWidth - margins.left - margins.right);
  ...
  if (yPos < signatureY - 10) {
    ...
    if (signataire?.isInterim) {
      doc.text('Le Directeur Général P.I.', pageWidth - margins.right - 60, signatureY + 8);
    ...
    body: [
      [
        ...
        `Note validée par: ${validatorName}\n\nStatut: ${
          note.statut === 'dg_valide' || note.statut === 'diffusee'
            ? 'VALIDÉE'
...
export async function generateNoteDGPdf(options: GeneratePdfOptions): Promise<PdfGenerationResult> {
  const { note, imputations, qrToken, baseUrl = window.location.origin, signataire } = options;
  ...
  await generatePage1(doc, note, imputations, qrDataUrl, logoDataUrl, signataire);

  // Page 2: Observations du DG
  await generatePage2(doc, note, qrDataUrl, verificationUrl, signataire);
