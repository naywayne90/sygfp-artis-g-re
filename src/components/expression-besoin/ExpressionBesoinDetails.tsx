        ...
        <Tabs defaultValue="infos" className="w-full">
          ...
          <TabsContent value="infos" className="mt-4 space-y-4">
                      ...
                      return (
                        <div key={step.order} className="flex items-center gap-2 flex-1">
                          ...
                          {index < VALIDATION_STEPS.length - 1 && (
                            <div
                              className={`h-0.5 w-4 ${isCompleted ? 'bg-success' : 'bg-muted'}`}
                            />
                          )}
                        </div>
                      );
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {expression.imputation.montant
                          ? new Intl.NumberFormat('fr-FR').format(expression.imputation.montant) +
                            ' FCFA'
                          : '-'}
                      </p>
                    </div>
            ...
            {expression.liste_articles &&
              Array.isArray(expression.liste_articles) &&
              expression.liste_articles.length > 0 && (
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base flex items-center gap-2">
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {expression.liste_articles.map(
                        (item: ExpressionBesoinLigne, index: number) => (
                          <div
                            key={index}
                            className="flex justify-between items-center p-2 bg-muted/50 rounded"
                          >
                            <span className="font-medium">{item.designation || item.article}</span>
                            <div className="flex items-center gap-4 text-sm text-muted-foreground">
                              ...
                              {item.prix_unitaire > 0 && (
                                <span className="font-medium text-foreground">
                                  {new Intl.NumberFormat('fr-FR').format(item.prix_total)} FCFA
                                </span>
                              )}
                            </div>
                          </div>
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {expression.calendrier_debut
                          ? format(new Date(expression.calendrier_debut), 'dd MMMM yyyy', {
                              locale: fr,
                            })
                          : '-'}
                      </p>
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {expression.calendrier_fin
                          ? format(new Date(expression.calendrier_fin), 'dd MMMM yyyy', {
                              locale: fr,
                            })
                          : '-'}
                      </p>
