...
export function ChaineDepenseKPIs({ stats, budgetGlobal }: ChaineDepenseKPIsProps) {
  ...
  const tauxLiquidation =
    stats.engagements.montant > 0
      ? (stats.liquidations.montant / stats.engagements.montant) * 100
      : 0;
  const tauxOrdonnancement =
    stats.liquidations.montant > 0
      ? (stats.ordonnancements.montant / stats.liquidations.montant) * 100
      : 0;
  const tauxPaiement =
    stats.ordonnancements.montant > 0
      ? (stats.reglements.montant / stats.ordonnancements.montant) * 100
      : 0;
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Vue d'ensemble de l'exécution budgétaire par étape</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {ETAPES.map((etape, index) => {
              ...
              return (
                <div key={etape.key} className="relative">
                  <Link to={etape.href}>
                    <div
                      className={`p-4 rounded-lg ${etape.bgColor} hover:shadow-md transition-all cursor-pointer border border-transparent hover:border-primary/20`}
                    >
                      ...
                      <p className="text-lg font-bold">{formatCurrency(montant)}</p>
                    </div>
                  </Link>
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {formatCurrency(stats.engagements.montant)} / {formatCurrency(budgetGlobal)}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {formatCurrency(stats.liquidations.montant)} /{' '}
                {formatCurrency(stats.engagements.montant)}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {formatCurrency(stats.ordonnancements.montant)} /{' '}
                {formatCurrency(stats.liquidations.montant)}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {formatCurrency(stats.reglements.montant)} /{' '}
                {formatCurrency(stats.ordonnancements.montant)}
              </p>
            </div>
          </div>
      ...
      {/* Alertes Pipeline */}
      {(stats.notes.aValider > 5 ||
        stats.engagements.aValider > 3 ||
        stats.liquidations.aValider > 3) && (
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  {stats.notes.aValider > 5 &&
                    `${stats.notes.aValider} notes en attente de validation. `}
                  {stats.engagements.aValider > 3 &&
                    `${stats.engagements.aValider} engagements à valider. `}
                  {stats.liquidations.aValider > 3 &&
                    `${stats.liquidations.aValider} liquidations en attente.`}
                </p>
              </div>
            </div>
          </CardContent>
