...
import { FolderOpen, Clock, Wallet, Building2, Target, AlertTriangle } from 'lucide-react';
  ...
  const { data: dossierStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        bloques:
          data?.filter((d) => d.statut_global === 'differe' || d.statut_global === 'rejete')
            .length || 0,
      };
    },
    enabled: !!exercice,
      ...
      if (engagements?.length) {
        const totalDays = engagements.reduce((sum, e) => {
          ...
          return (
            ...
            Math.max(
              0,
              Math.floor((validated.getTime() - created.getTime()) / (1000 * 60 * 60 * 24))
            )
          );
        }, 0);
    ...
    queryFn: async () => {
      ...
      budgetLines?.forEach((bl) => {
        if (bl.direction_id && bl.directions) {
          ...
          const existing = dirMap.get(bl.direction_id) || {
            code: dir.code,
            label: dir.label,
            dotation: 0,
          };
  ...
  const { data: disponibilite } = useQuery({
    ...
    queryFn: async () => {
      ...
      const tauxDisponibilite =
        dotationTotale > 0 ? Math.round((Math.max(0, disponibleRaw) / dotationTotale) * 100) : 0;
              ...
              <div className="text-center py-2">
                ...
                {(disponibilite?.engageTotal || 0) > 0 && (
                  <p className="text-xs text-destructive mt-1">
                    ⚠️ {formatCurrency(disponibilite?.engageTotal || 0)} engagé sans dotation
                  </p>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-destructive">
                    -{formatCurrency(Math.abs(disponibilite?.disponible || 0))}
                  </span>
                </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">
                    {formatCurrency(disponibilite?.disponible || 0)}
                  </span>
                </div>
              ...
              <div className="space-y-2">
                {osStats?.map((os) => (
                  <div key={os.id} className="flex items-center justify-between text-sm">
                    <span className="truncate max-w-[120px]" title={os.label}>
                      {os.code}
                    </span>
                    <span className="font-medium">{formatCurrency(os.dotation)}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-2">
                {directionStats?.map((dir) => (
                  <div key={dir.id} className="flex items-center justify-between text-sm">
                    <span className="truncate max-w-[120px]" title={dir.label}>
                      {dir.code}
                    </span>
                    <span className="font-medium">{formatCurrency(dir.dotation)}</span>
                  </div>
                ))}
              </div>
            )}
