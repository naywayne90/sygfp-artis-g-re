...
export function ImputationSummaryCard({
  ...
  const tauxEngagement =
    dotationActuelle > 0 ? ((dotationActuelle - disponibleApres) / dotationActuelle) * 100 : 0;

  const tauxEngagementActuel = dotationActuelle > 0 ? (cumulEngage / dotationActuelle) * 100 : 0;
  ...
  return (
    <TooltipProvider>
      <Card
        className={`${isInsufficient && !isForced ? 'border-destructive' : isValid ? 'border-green-500' : ''}`}
      >
        ...
        <CardContent className="space-y-4">
            ...
            <div className="bg-blue-50 dark:bg-blue-950/20 rounded-lg p-3 text-sm space-y-2">
              ...
              <div className="space-y-1 pl-6">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono">{formatCurrency(dotationInitiale)}</span>
                </div>
                {virementsRecus > 0 && (
                  <div className="flex justify-between text-green-600">
                    <span>+ Virements reçus:</span>
                    <span className="font-mono">+{formatCurrency(virementsRecus)}</span>
                  </div>
                )}
                {virementsEmis > 0 && (
                  <div className="flex justify-between text-red-600">
                    <span>− Virements émis:</span>
                    <span className="font-mono">−{formatCurrency(virementsEmis)}</span>
                  </div>
                )}
                ...
                <div className="flex justify-between font-medium">
                  ...
                  <span className="font-mono text-blue-600">
                    {formatCurrency(dotationActuelle || calculatedDotationActuelle)}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-muted/50 rounded-lg p-3 text-sm space-y-2">
            ...
            <div className="space-y-1 pl-6">
              <div className="flex justify-between">
                ...
                <span className="font-mono">{formatCurrency(dotationActuelle)}</span>
              </div>
              <div className="flex justify-between text-orange-600">
                ...
                <span className="font-mono">
                  −
                  {formatCurrency(
                    cumulEngage || dotationActuelle - disponibleAvant - montantReserve
                  )}
                </span>
              </div>
              <Separator className="my-1" />
              <div className="flex justify-between">
                ...
                <span className={`font-mono ${disponibleBrut < 0 ? 'text-destructive' : ''}`}>
                  {formatCurrency(disponibleBrut || disponibleAvant + montantReserve)}
                </span>
              </div>
              <div className="flex justify-between text-amber-600">
                ...
                <span className="font-mono">−{formatCurrency(montantReserve)}</span>
              </div>
              <Separator className="my-1" />
              <div className="flex justify-between font-bold">
                <span>= Disponible net:</span>
                <span
                  className={`font-mono ${disponibleAvant < 0 ? 'text-destructive' : 'text-green-600'}`}
                >
                  {formatCurrency(disponibleAvant)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <span
              className={`font-mono text-xs w-12 text-right ${tauxEngagementActuel > 100 ? 'text-destructive' : tauxEngagementActuel > 80 ? 'text-orange-600' : ''}`}
            >
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm pt-2 border-t">
              ...
              <span
                className={`font-mono font-medium ${disponibleApres < 0 ? 'text-destructive' : 'text-green-600'}`}
              >
            ...
            <div className="flex justify-between text-sm">
              ...
              <span
                className={`font-mono ${tauxEngagement > 100 ? 'text-destructive' : tauxEngagement > 80 ? 'text-orange-600' : ''}`}
              >
          ...
          {isInsufficient && (
            <Alert variant={isForced ? 'default' : 'destructive'}>
              ...
              <AlertDescription>
                Le montant demandé ({formatCurrency(montantImpute)}) dépasse le disponible net (
                {formatCurrency(disponibleAvant)}).
                {isForced ? (
