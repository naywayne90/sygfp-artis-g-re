...
export function BudgetImportWizard() {
  ...
  const {
    ...
    createImportRun,
    loadStagingData,
    validateImportRun: _validateImportRun,
    executeImport,
  } = useImportStaging();
  const {
  ...
  const {
    canImport,
    isBudgetLockedForUser: _isBudgetLockedForUser,
  ...
  const [selectedExercice, setSelectedExercice] = useState<number>(
  ...
  const handleFileUpload = useCallback(
    async (uploadedFile: File) => {
      ...
      try {
        ...
        const groupeSheet = parsedSheets.find(
          (s) =>
  ...
  const validateData = useCallback(async () => {
    ...
    parsedRows.forEach((row) => {
      ...
      row.errors.forEach((errorMsg) => {
        errors.push({
          row: row.rowNumber,
          column: errorMsg.includes('Imputation')
            ? 'imputation'
            : errorMsg.includes('Montant')
              ? 'montant'
              : 'général',
          message: errorMsg,
          severity: 'error',
    ...
    if (!hasBlockingErrors && file && selectedSheet) {
      ...
      if (newRunId) {
        ...
        console.warn(
          `[AUDIT] Import préparé: exercice=${selectedExercice}, fichier=${file.name}, lignes=${parsedRows.length}`
        );
      }
    }
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      // Log import result
      await logImportAction(runId, file?.name || 'unknown', result.success ? 'success' : 'error', {
        totalRows: parsedRows.length,
      ...
      if (result.success) {
        toast.success(
          `Import réussi: ${result.insertedCount} créées, ${result.updatedCount} mises à jour`
        );
      } else {
        toast.warning(
          `Import partiel: ${result.importedCount} réussies, ${result.errorCount} erreurs`
        );
      }
    } catch (error) {
  ...
  const handleNext = useCallback(async () => {
    ...
    if (currentStep === 2 && syncReferentielsEnabled && !refsSynced && file) {
      ...
      if (result.summary.totalInserted + result.summary.totalUpdated > 0) {
        toast.success(
          `Référentiels synchronisés: ${result.summary.totalInserted} créés, ${result.summary.totalUpdated} mis à jour`
        );
        refreshDropdowns();
      }
      setRefsSynced(true);
  ...
  // Show loading skeleton while checking import permissions
  ...
  if (!canImport && importBlockReason) {
    const isRoleIssue = !hasImportOverrideRole && !budgetValidation?.isValidated;
    return (
      <Alert variant="destructive" className="my-4">
        ...
        <AlertTitle>
          {isRoleIssue
            ? 'Droits insuffisants'
            : budgetValidation?.isValidated
              ? 'Budget validé — Import restreint'
              : 'Import bloqué'}
        </AlertTitle>
        <AlertDescription className="mt-1">{importBlockReason}</AlertDescription>
      </Alert>
    );
  }
