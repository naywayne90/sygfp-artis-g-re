...
export function TopBar() {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 flex h-14 items-center gap-2 border-b bg-card px-3 lg:px-6 shadow-sm">
        ...
        <div className="hidden lg:flex items-center gap-3 mr-4">
          <div className="flex h-9 items-center justify-center rounded-lg bg-white px-2 shadow-sm border">
            <img src={logoArti} alt="ARTI" className="h-6 w-auto object-contain" />
          </div>
        ...
        {badges && badges.totalATraiter > 0 && (
          ...
          >
            ...
            <span className="text-xs font-medium">{badges.totalATraiter} à traiter</span>
          </Button>
        )}
      ...
      <ExerciceChangeModal open={showExerciceModal} onOpenChange={setShowExerciceModal} />

      {/* Dialog recherche de dossiers - rendu conditionnel pour éviter les requêtes inutiles */}
      {showSearchDialog && (
        <DossierSearchDialog open={showSearchDialog} onOpenChange={setShowSearchDialog} />
      )}
    </>
  );
}
