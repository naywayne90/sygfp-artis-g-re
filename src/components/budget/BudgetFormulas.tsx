...
import { BudgetOverviewSheet } from './BudgetOverviewSheet';
...
const BUDGET_FORMULAS: FormulaItem[] = [
  ...
  },
  {
    ...
    example: 'Ex: (20 000 000 FCFA payés / 100 000 000 FCFA) × 100 = 20%',
  },
  {
    id: 'taux_engagement',
    ...
    example: 'Ex: (30 000 000 FCFA engagés / 105 000 000 FCFA) × 100 = 28,6%',
  },
];
...
export function BudgetFormulas({ className, compact = false }: BudgetFormulasProps) {
  ...
  const [sheetTab, setSheetTab] = useState<'dotation' | 'disponible'>('dotation');
  const { isDG, isCB, isDAF, isSousDirecteur } = useRBAC();
  const { hasAnyRole } = usePermissions();

  // DG, DAAF, sous-directeur DAAF, CB peuvent consulter les données budget
  // Double vérification : RBACContext (profil_fonctionnel) + usePermissions (user_roles)
  const canViewBudget =
    isDG || isDAF || isCB || isSousDirecteur || hasAnyRole(['DG', 'DAAF', 'DAF', 'CB', 'ADMIN']);

  const handleBadgeClick = (formulaId: string) => {
    setSheetTab(formulaId === 'disponible' ? 'disponible' : 'dotation');
    setSheetOpen(true);
  };
  ...
  if (compact) {
    return (
      <>
                ...
                <TooltipTrigger asChild>
                  <Badge
                    ...
                    className={`transition-colors ${
                      canViewBudget
                        ? 'cursor-pointer hover:bg-primary/10 hover:border-primary/40 hover:text-primary'
                        : 'cursor-default hover:bg-muted'
                    }`}
                    onClick={canViewBudget ? () => handleBadgeClick(formula.id) : undefined}
                    role={canViewBudget ? 'button' : undefined}
                  >
                  ...
                  <div className="space-y-1">
                    ...
                    {canViewBudget && (
                      <p className="text-xs text-primary font-medium mt-1">
                        Cliquez pour voir les données réelles
                      </p>
                    )}
                  </div>
                </TooltipContent>
        ...
        <BudgetOverviewSheet open={sheetOpen} onOpenChange={setSheetOpen} defaultTab={sheetTab} />
      </>
    );
  }
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-blue-700 dark:text-blue-400 text-xs mt-1">
                      Ces formules sont appliquées automatiquement par le système. Le contrôle de
                      disponibilité est effectué lors de chaque engagement ou virement. Un
                      engagement ne peut être validé que si le disponible est suffisant.
                    </p>
                  </div>
                </div>
              </div>
