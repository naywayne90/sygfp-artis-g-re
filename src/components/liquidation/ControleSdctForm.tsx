...
const controleSdctSchema = z.object({
...
const CHECKLIST_ITEMS = [
  ...
  { key: "calcul_retenues_ok", label: "Calcul des retenues correct (TVA, AIRSI, etc.)", icon: Shield },
];

export function ControleSdctForm({
  liquidationId,
  liquidation,
  onSuccess,
}: ControleSdctFormProps) {
  ...
  // Seul SDCT peut faire le contrôle
  const canControl = hasAnyRole(["ADMIN", "SDCT"]);
  ...
  const allChecked = form.watch("montant_verifie") &&
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Après SDCT, ça peut aller à DG si montant > seuil ou directement valide
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          workflow_status: isValidation ? (needsDgValidation ? "validation_dg" : "termine") : "rejete",
          ...(isValidation ? {} : {
            rejection_reason: formData.motif_rejet,
            rejected_at: new Date().toISOString(),
            rejected_by: user.id,
          }),
          ...(isValidation && !needsDgValidation ? {
      ...
      toast.success(
        isValidation
          ? (needsDgValidation
              ? "Contrôle SDCT validé - Transmis au DG pour validation"
              : "Liquidation validée avec succès")
          : "Liquidation rejetée"
      );
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Le service fait doit être certifié avant le contrôle SDCT.
              </p>
            </div>
          </div>
        </CardContent>
          ...
          <div className="bg-muted/50 rounded-lg p-4 mb-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-mono text-xs">{liquidation.engagement?.budget_line?.code}</span>
              </div>
              <div>
                ...
                <div className="flex items-start gap-2 p-3 bg-warning/10 border border-warning/20 rounded-lg">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-muted-foreground">
                      Seul le rôle SDCT peut effectuer ce contrôle.
                    </p>
                  </div>
                </div>
              )}
              ...
              {liquidation.montant >= 50000000 && (
                <div className="bg-secondary/10 border border-secondary/20 rounded-lg p-3 text-sm">
                  ...
                  <p className="text-muted-foreground mt-1">
                    Cette liquidation nécessitera une validation DG après le contrôle SDCT.
                  </p>
                </div>
              )}
            </form>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-3">
                <p>
                  {pendingDecision === "valider"
                    ? `Vous confirmez que le contrôle SDCT de la liquidation ${liquidation.numero} est conforme.`
                ...
                {pendingDecision === "valider" && liquidation.montant >= 50000000 && (
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              className={pendingDecision === "rejeter" ? "bg-destructive hover:bg-destructive/90" : ""}
            >
              Confirmer
            </AlertDialogAction>
          </AlertDialogFooter>
