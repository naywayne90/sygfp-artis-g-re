...
import { usePassationsMarche, MODES_PASSATION, EBValidee } from "@/hooks/usePassationsMarche";
import { usePrestataires } from "@/hooks/usePrestataires";
import { useExercice } from "@/contexts/ExerciceContext";
import { FileText, Search, Plus, Trash2, Loader2, Users, ClipboardList, Gavel, Building2 } from "lucide-react";
...
export function PassationMarcheForm({
  ...
  const resetForm = () => {
    ...
    setSearchEB("");
    setActiveStep("mode");
    setFormData({ mode_passation: "gre_a_gre", type_procedure: "" });
          ...
          <>
            ...
            <Tabs value={activeStep} onValueChange={setActiveStep} className="w-full">
              ...
              <TabsContent value="decision" className="mt-4 space-y-4">
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base">Sélection du prestataire</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {prestatairesSollicites.length === 0 ? (
                      <p className="text-sm text-muted-foreground">
                        Aucun prestataire sollicité. Retournez à l'onglet Prestataires.
                      </p>
                    ) : (
                      <div className="space-y-2">
                        {prestatairesSollicites.map((ps) => (
                          <div
                            ...
                            onClick={() => {
                              // Désélectionner tous, sélectionner celui-ci
                              setPrestatairesSollicites((prev) =>
                                prev.map((p) => ({
                                  ...p,
