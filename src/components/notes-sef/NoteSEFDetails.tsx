...
export function NoteSEFDetails({
  ...
  const canModify = (isCreator || isAdmin) && note?.statut === 'soumis';
  const canSubmit = canModify;
  const canAddAttachments = canModify;
  const canValidateNote =
    canValidate &&
    (note?.statut === 'soumis' || note?.statut === 'a_valider' || note?.statut === 'differe');
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const { error: dbError } = await supabase.from('notes_sef_pieces').insert({
          note_id: note.id,
          nom: file.name,
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {note.direction?.label || note.direction?.sigle || 'Non spécifiée'}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start gap-2">
                ...
                <CardContent>
                  ...
                  {note.rejected_at && (
                    <p className="text-sm text-muted-foreground mt-2">
                      Rejetée le{' '}
                      {format(new Date(note.rejected_at), 'dd MMM yyyy à HH:mm', { locale: fr })}
                    </p>
                  )}
                </CardContent>
              </Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  {note.differe_date_reprise && (
                    <div>
                      ...
                      <p>
                        {format(new Date(note.differe_date_reprise), 'dd MMM yyyy', { locale: fr })}
                      </p>
                    </div>
                  )}
                  {note.differe_at && (
                    <p className="text-sm text-muted-foreground mt-2">
                      Différée le{' '}
                      {format(new Date(note.differe_at), 'dd MMM yyyy à HH:mm', { locale: fr })}
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card className="border-success/50">
                ...
                <CardContent className="space-y-3">
                  {note.validated_at && (
                    <p className="text-sm text-muted-foreground">
                      Validée le{' '}
                      {format(new Date(note.validated_at), 'dd MMM yyyy à HH:mm', { locale: fr })}
                    </p>
                  )}
                        ...
                        <div key={entry.id} className="flex gap-4 relative">
                          ...
                          <div
                            className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center z-10 ${
                              entry.new_statut === 'valide'
                                ? 'bg-success text-success-foreground'
                                : entry.new_statut === 'rejete'
                                  ? 'bg-destructive text-destructive-foreground'
                                  : entry.new_statut === 'differe'
