...
import { User, Shield, Key, Building2, CheckCircle, Users, Clock } from 'lucide-react';
...
export default function MonProfil() {
  const { userId, userRoles, isAdmin, getAllPermissions } = usePermissions();
  ...
  const { data: profile } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*, direction:directions!profiles_direction_id_fkey(code, label)')
        .eq('id', userId)
        .single();
      ...
      {delegationsRecues.length > 0 && (
        <Card className="border-primary/50">
          <CardHeader>
            ...
            <CardDescription>Vous agissez pour le compte d'autres utilisateurs</CardDescription>
          </CardHeader>
          <CardContent>
