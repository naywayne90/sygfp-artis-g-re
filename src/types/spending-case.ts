...
// Statut global du dossier (brouillon supprime — creation = soumis)
export type SpendingCaseStatus =
  | 'soumis' // Soumis (statut initial, plus de brouillon)
  | 'in_progress' // En cours
  | 'blocked' // Bloque (rejet ou probleme)
  | 'completed' // Termine
  | 'cancelled'; // Annule
...
export const VALID_TRANSITIONS: TransitionRule[] = [
  {
    ...
    to: 'note_aef',
    condition: (sc) =>
      sc.timeline.steps.find((s) => s.stage === 'note_sef')?.status === 'completed',
  ...
  },
  {
    ...
    to: 'imputation',
    condition: (sc) =>
      sc.timeline.steps.find((s) => s.stage === 'note_aef')?.status === 'completed',
    requiredRole: 'CB',
    requiresValidation: true,
  },
  {
  ...
  },
  {
    ...
    to: 'engagement',
    condition: (sc) =>
      sc.timeline.steps.find((s) => s.stage === 'passation_marche')?.status === 'completed',
    requiresValidation: true,
  },
  {
    from: 'engagement',
    to: 'liquidation',
    condition: (sc) =>
      sc.timeline.steps.find((s) => s.stage === 'engagement')?.status === 'completed',
    requiresValidation: true,
  },
  {
    from: 'liquidation',
    to: 'ordonnancement',
    condition: (sc) =>
      sc.timeline.steps.find((s) => s.stage === 'liquidation')?.status === 'completed',
    requiredRole: 'DG',
    requiresValidation: true,
  },
  {
    ...
    to: 'reglement',
    condition: (sc) =>
      sc.timeline.steps.find((s) => s.stage === 'ordonnancement')?.status === 'completed',
...
export function getStageStatus(spendingCase: SpendingCase, stage: SpendingStage): StepStatus {
  const stepData = spendingCase.timeline.steps.find((s) => s.stage === stage);
  return stepData?.status || 'pending';
}

export function isStageComplete(spendingCase: SpendingCase, stage: SpendingStage): boolean {
