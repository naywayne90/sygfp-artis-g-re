...
export function useNotesSEFAudit() {
  ...
  const logAuditEvent = useCallback(
    async ({ noteId, action, oldStatut, newStatut, commentaire, details }: AuditParams) => {
      try {
        const {
          data: { user },
  ...
  /**
   * Notifications gérées exclusivement par les triggers DB (Gap 4).
   * - notify_on_notes_sef_status_change() gère DG + délégataires + intérimaires
   * - notify_role() utilise get_users_who_can_act_as_role() pour tous les modules
   * Ces fonctions restent exportées pour ne pas casser les signatures de logSoumission etc.
   */
  const getValidators = useCallback(async (): Promise<string[]> => {
    return [];
  }, []);

  const sendNotifications = useCallback(async (_params: NotificationParams) => {
    // No-op: les triggers DB gèrent toutes les notifications
    // incluant les délégataires et intérimaires (Gap 4)
  }, []);

  /**
   * Log création
   */
  const logCreation = useCallback(
    async (noteId: string, details?: Json) => {
      await logAuditEvent({
        noteId,
        action: 'creation',
        newStatut: 'brouillon',
   ...
   * Log modification
   */
  const logModification = useCallback(
    async (noteId: string, oldStatut: string | null, details?: Json) => {
      await logAuditEvent({
        noteId,
        action: 'modification',
        oldStatut,
   ...
   * Log ajout pièce
   */
  const logAjoutPiece = useCallback(
    async (noteId: string, nomFichier: string) => {
      await logAuditEvent({
        noteId,
        action: 'ajout_piece',
   ...
   */
  const logSuppressionPiece = useCallback(
    async (noteId: string, nomFichier: string) => {
      await logAuditEvent({
        noteId,
   ...
   */
  const logReferenceGenerated = useCallback(
    async (noteId: string, reference: string) => {
      await logAuditEvent({
        noteId,
   ...
   */
  const logSoumission = useCallback(
    async (note: NoteSEF) => {
  ...
  const logValidation = useCallback(
    async (note: NoteSEF, dossierId?: string) => {
      await logAuditEvent({
        noteId: note.id,
        action: 'validation',
   ...
   */
  const logRejet = useCallback(
    async (note: NoteSEF, motif: string) => {
      await logAuditEvent({
        noteId: note.id,
        action: 'rejet',
        oldStatut: note.statut,
  ...
  const logReport = useCallback(
    async (note: NoteSEF, motif: string, dateReprise?: string) => {
      await logAuditEvent({
        noteId: note.id,
        action: 'report',
        oldStatut: note.statut,
