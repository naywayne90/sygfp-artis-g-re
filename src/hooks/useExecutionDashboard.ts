...
export function useExecutionDashboard() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ExecutionStats> => {
      ...
      const [engagementsRes, liquidationsRes, ordonnancementsRes, reglementsRes, dossiersRes] =
        await Promise.all([
          supabase
      ...
      const evolutionMensuelle = MOIS.map((mois, index) => {
        ...
        return {
          mois,
          engagements: filterByMonth(
            engagements.filter((e) => e.statut === 'valide'),
            'date_engagement'
          ).reduce((sum, e) => sum + ((e as any).montant || 0), 0),
          liquidations: filterByMonth(
            liquidations.filter((l) => l.statut === 'validé_dg'),
            'date_liquidation'
          ).reduce((sum, l) => sum + ((l as any).montant || 0), 0),
          ordonnancements: filterByMonth(
            ordonnancements.filter((o: any) => o.statut === 'signe' || o.statut === 'valide'),
            'created_at'
          ).reduce((sum, o) => sum + ((o as any).montant || 0), 0),
          reglements: filterByMonth(
            reglements.filter((r: any) => r.statut === 'paye'),
            'created_at'
