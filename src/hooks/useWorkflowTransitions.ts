...
export function useWorkflowTransitions(options: UseWorkflowTransitionsOptions) {
  const { module, entityId, currentStatus, entityCode, onTransitionSuccess, onTransitionError } =
    options;
  ...
  const {
    ...
    queryFn: () => getAvailableTransitions(module, currentStatus || 'soumis'),
  ...
  const checkTransition = useCallback(
    async (toStatus: string) => {
      if (!currentStatus)
        return { allowed: false, reason: 'Statut actuel inconnu', requiresMotif: false };
  ...
  const executeTransition = useCallback(
    (toStatus: string, motif?: string, metadata?: Record<string, unknown>) => {
      if (!currentStatus) {
  ...
  const findTransition = useCallback(
    (toStatus: string): WorkflowTransition | undefined => {
