...
export const VALIDATION_STEPS = [
  ...
  { order: 2, role: 'SDPM', label: 'SDPM', description: 'Validation SDPM' },
  {
    ...
    role: 'SDCT',
    label: 'SDCT',
    description: 'Vérification conformité fournisseur/procédure',
  },
...
export function useMarches() {
  ...
  const fetchMarches = async (filters?: {
    statut?: string;
    ...
    let filtered = (data || []) as Marche[];

    if (filters?.statut) {
      filtered = filtered.filter((m) => m.statut === filters.statut);
    }
    if (filters?.validation_status) {
      filtered = filtered.filter((m) => m.validation_status === filters.validation_status);
    }
  ...
  const validateStepMutation = useMutation({
    mutationFn: async ({
      ...
      if (isLastStep) {
        ...
        await supabase
          .from('marches')
          .update({ current_validation_step: nextStep })
          .eq('id', marcheId);
