...
export function useExport() {
  ...
  const downloadFile = useCallback(
    (content: string | Blob, fileName: string, contentType: string) => {
  ...
  const exportViaEdgeFunction = useCallback(
  ...
  const exportToExcel = useCallback(
    async (data: Record<string, unknown>[], fileName: string, sheetName = 'Données') => {
      setIsExporting(true);
      try {
  ...
  const exportBudgetLines = useCallback(
    async (filters?: Record<string, unknown>, includeReferentiels = false) => {
      setIsExporting(true);
      try {
        // Fetch data
        let query = supabase
        ...
        if (includeReferentiels) {
          ...
          if (osRes.data?.length) {
            const osSheet = XLSX.utils.json_to_sheet(
              osRes.data.map((o: { code: string; libelle: string }) => ({
                'Code OS': o.code,
                Libellé: o.libelle,
              }))
            );
          ...
          if (actionsRes.data?.length) {
            const actionsSheet = XLSX.utils.json_to_sheet(
              actionsRes.data.map((a: { code: string; libelle: string }) => ({
                'Code Action': a.code,
                Libellé: a.libelle,
              }))
            );
          ...
          if (directionsRes.data?.length) {
            const dirsSheet = XLSX.utils.json_to_sheet(
              directionsRes.data.map(
                (d: { code: string; label: string; sigle: string | null }) => ({
                  Code: d.code,
                  Libellé: d.label,
                  Sigle: d.sigle,
                })
          ...
          if (nbeRes.data?.length) {
            const nbeSheet = XLSX.utils.json_to_sheet(
              nbeRes.data.map((n: { code: string; libelle: string }) => ({
                'Code NBE': n.code,
                Libellé: n.libelle,
              }))
            );
