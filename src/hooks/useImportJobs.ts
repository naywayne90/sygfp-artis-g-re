...
export function useImportJobs() {
  ...
  const uploadFile = useCallback(async (jobId: string, file: File): Promise<string | null> => {
    // Check file size
  ...
  const storeImportRows = useCallback(
    async (jobId: string, parsedRows: ParsedExcelRow[]): Promise<boolean> => {
      setIsLoading(true);
      try {
        ...
        const rowsToInsert = parsedRows.map((row) => ({
          ...
          status: row.isValid ? (row.warnings.length > 0 ? 'warning' : 'ok') : 'error',
  ...
  const executeImport = useCallback(
    async (
      ...
      try {
        ...
        const finalStatus =
          errors === 0 ? 'completed' : inserted + updated > 0 ? 'completed' : 'failed';
