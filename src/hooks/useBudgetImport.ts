...
export function useBudgetImport() {
  ...
  const finalizeImport = useMutation({
    mutationFn: async (params: {
      ...
      const status = params.errorRows > 0
        ? (params.successRows > 0 ? "partiel" : "echec")
        : "termine";
  ...
  const validateImportData = useCallback(async (
    ...
    const [directions, objectifs, missions, actions, existingLines] = await Promise.all([
  ...
  const executeImport = useCallback(async (
    ...
    try {
      // 2. Fetch reference maps
      const [directions, objectifs, missions, actions, activites, existingLines] = await Promise.all([
        supabase.from("directions").select("id, code"),
        supabase.from("objectifs_strategiques").select("id, code"),
        supabase.from("missions").select("id, code"),
        supabase.from("actions").select("id, code"),
      ...
      if (toInsert.length > 0) {
        ...
        if (insertError) {
          ...
          await supabase
            ...
            .update({
              status: "echec",
              errors: [{ row: 0, message: `Échec de l'import: ${insertError.message}` }],
              completed_at: new Date().toISOString(),
            })
            .eq("id", importRecord.id);

          throw new Error(`Import annulé: ${insertError.message}`);
        }
      ...
      for (const { id, data } of toUpdate) {
        ...
        if (updateError) {
          errorCount++;
          errors.push({ row: 0, message: `Mise à jour ${data.code}: ${updateError.message}` });
        } else {
          successCount++;
        }
      }
