...
import {
  downloadNoteDGPdf,
  generateNoteDGPdf,
} from "@/services/noteDGPdfService";
import {
...
export function useNoteDGPdf({ noteId }: UseNoteDGPdfOptions) {
  ...
  const generateAndStorePdf = useMutation({
    mutationFn: async ({ note, imputations }: GeneratePdfParams) => {
      ...
      try {
        ...
        // TODO: Stocker le PDF via Edge Function r2-storage
        // Pour l'instant, on retourne juste le blob
        // Dans une implémentation complète, on uploaderait vers Cloudflare R2
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            pdf_hash: pdfHash,
            // pdf_url: storedUrl, // À implémenter avec R2
          })
          .eq("id", note.id);
        ...
        await logAction({
          ...
          entityId: note.id,
          action: "pdf_generate",
          newValues: { pdf_hash: pdfHash, qr_token: qrToken },
        });
        ...
        return { success: true, qrToken, filename, pdfHash };
      } finally {
        setIsGenerating(false);
      }
    },
