...
/**
 * Fonction utilitaire pour vérifier si un engagement est possible
 */
export async function checkEngagementPossible(
  budgetLineId: string,
  montant: number,
  exercice: number
): Promise<{ possible: boolean; disponible: number; message: string }> {
  // Récupérer la ligne
  ...
  if (!line) {
    return { possible: false, disponible: 0, message: "Ligne budgétaire introuvable" };
  }
  ...
  if (montant > disponible) {
    return {
      possible: false,
      disponible,
      message: `Disponible insuffisant: ${disponible.toLocaleString("fr-FR")} FCFA disponibles, ${montant.toLocaleString("fr-FR")} FCFA demandés`,
    };
  }
...
/**
 * Fonction utilitaire pour vérifier si un virement est possible
 */
export async function checkVirementPossible(
  ...
  if (!line) {
    return { possible: false, disponible: 0, message: "Ligne source introuvable" };
  }
  ...
  if (montant > disponible) {
    return {
      possible: false,
      disponible,
      message: `Virement impossible: ${disponible.toLocaleString("fr-FR")} FCFA disponibles sur la ligne source`,
    };
  }
