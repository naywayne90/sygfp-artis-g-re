...
export function useReferentielsValidation() {
   ...
   */
  const validateHierarchie = useCallback(
    async (
   ...
   */
  const canLinkActiviteToPlan = useCallback(
    async (
      activiteId: string,
   ...
   */
  const createImportLog = useCallback(
    async (
   ...
   */
  const finalizeImportLog = useCallback(
    async (
      importId: string,
      results: {
   ...
   */
  const getPlansTravailByDirection = useCallback(
    async (directionId: string, exerciceId?: string): Promise<PlanTravail[]> => {
      try {
        let query = supabase
          .from('plans_travail')
          .select('*')
   ...
   */
  const getDirectionOSMapping = useCallback(
    async (
      directionId?: string,
      osId?: string,
      exerciceId?: string
      ...
      try {
        let query = supabase.from('direction_os_mapping').select('*').eq('est_actif', true);
   ...
   */
  const validateDepenseBeforeImputation = useCallback(
    async (params: {
      activiteId?: string;
      actionId?: string;
      osId?: string;
      ...
      if (!hierarchie.valid) {
        errors.push(...hierarchie.results.filter((r) => !r.est_coherent).map((r) => r.message));
      }
