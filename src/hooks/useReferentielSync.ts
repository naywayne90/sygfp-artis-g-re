...
export function useReferentielSync() {
  ...
  const importOS = useCallback(
    async (workbook: XLSX.WorkBook, sheetNames: string[]): Promise<ReferentielImportResult> => {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          if (existing) {
            ...
            await supabase.from('objectifs_strategiques').insert({
              code,
              libelle,
              est_actif: true,
      ...
      for (const row of rows) {
        ...
        try {
          ...
          if (existing) {
            ...
            await supabase
              .from('actions')
              .insert({ code, libelle, os_id: osId, mission_id: missionId, est_active: true });
            result.inserted++;
          }
        } catch (err) {
          result.errors++;
  ...
  const importDirections = useCallback(
    async (workbook: XLSX.WorkBook, sheetNames: string[]): Promise<ReferentielImportResult> => {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          if (existing) {
            ...
            await supabase.from('directions').insert({ code, label, est_active: true });
            result.inserted++;
          }
        } catch (err) {
          result.errors++;
  ...
  const importActivites = useCallback(
    async (workbook: XLSX.WorkBook, sheetNames: string[]): Promise<ReferentielImportResult> => {
  ...
  const importSousActivites = useCallback(
    async (workbook: XLSX.WorkBook, sheetNames: string[]): Promise<ReferentielImportResult> => {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          if (existing) {
            ...
            await supabase.from('nomenclature_nbe').insert({ code, libelle, est_active: true });
            result.inserted++;
          }
        } catch (err) {
          result.errors++;
  ...
  const importNatureDepense = useCallback(
    async (workbook: XLSX.WorkBook, sheetNames: string[]): Promise<ReferentielImportResult> => {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          if (existing) {
            ...
            await supabase.from('ref_nve').insert({ code_nve: code, libelle, actif: true });
            result.inserted++;
          }
        } catch (err) {
          result.errors++;
  ...
  const importAllReferentiels = useCallback(
    async (file: File): Promise<AllReferentielsResult> => {
      setIsSyncing(true);
  ...
  const createMissingReference = useCallback(
    async (
  ...
  const detectReferenceSheets = useCallback(
    (
      sheetNames: string[]
    ): {
