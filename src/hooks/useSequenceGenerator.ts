...
export const useSequenceGenerator = () => {
  ...
  const syncSequenceFromImport = useCallback(
    async (
      docType: DocType,
  ...
  const parseExistingCode = useCallback(
    async (
      code: string
    ): Promise<{
      prefix: string | null;
  ...
  const getNextSequenceForExercice = useCallback(
    async (
      docType: DocType,
  ...
  const updateSequenceCounter = useCallback(
    async (
      docType: DocType,
      newNumber: number,
      directionCode?: string,
      ...
      const { data, error } = await supabase.rpc(
        'update_sequence_counter' as never,
        {
          p_doc_type: docType,
      ...
      if (error)
        throw new Error(`Erreur mise à jour compteur: ${(error as { message: string }).message}`);
      return data === true;
    },
    [exercice]
  );
