...
export function useCoherenceCheck() {
  ...
  const generateReport = useMutation({
    mutationFn: async (
      source: 'import' | 'manual' | 'scheduled' = 'manual'
      ...
      const { error: insertError } = await (supabase.from as any)('coherence_reports').insert({
        ...
        anomalies_count: report.anomaliesCount,
        errors_count: report.errorsCount,
        warnings_count: report.warningsCount,
        infos_count: report.infosCount,
        anomalies: JSON.parse(JSON.stringify(allAnomalies)),
        status: report.status,
      });

      if (insertError) {
        console.error('Erreur sauvegarde rapport cohérence:', insertError);
      }
...
export function useCoherenceReports() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<CoherenceReport[]> => {
      ...
      if (error) {
        console.error('Erreur récupération rapports cohérence:', error);
        return [];
      }
...
/**
 * Hook pour mettre à jour le statut d'un rapport de cohérence
 */
export function useUpdateCoherenceReportStatus() {
  const queryClient = useQueryClient();
...
export function useImportValidation() {
  ...
  const validateBudgetImport = useMutation({
    mutationFn: async (data: {
      lignes: Array<{ code: string; montant_ae: number; montant_cp: number }>;
    }): Promise<Anomaly[]> => {
      const anomalies: Anomaly[] = [];
  ...
  const validateRoadmapImport = useMutation({
    mutationFn: async (data: {
      activites: Array<{ code: string; libelle: string; montant_prevu: number }>;
    }): Promise<Anomaly[]> => {
      const anomalies: Anomaly[] = [];
