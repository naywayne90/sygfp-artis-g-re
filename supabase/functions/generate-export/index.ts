...
function generatePDFHTML(data: any, entityType: string): string {
  const logoUrl = "https://tjagvgqthlibdpvztvaf.supabase.co/storage/v1/object/public/assets/logo-arti.jpg";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (body.type) {
      case "csv":
      case "excel": {
        // Fetch budget lines with filters
        let query = supabase
          .from("budget_lines")
          .select(`
        ...
        if (body.filters?.search) {
          query = query.or(`code.ilike.%${body.filters.search}%,label.ilike.%${body.filters.search}%`);
        }
        ...
        if (body.include_referentiels && body.type === "excel") {
          const [osRes, actionsRes, directionsRes, nbeRes] = await Promise.all([
            ...
            supabase.from("actions").select("code, libelle, os:objectifs_strategiques(code)").order("code"),
