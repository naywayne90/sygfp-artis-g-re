#!/usr/bin/env python3
"""
Script d'import du personnel ARTI dans SYGFP
...
def get_direction_ids():
    ...
    for d in result.data:
        # Préférer les directions dont le code = sigle (les principales)
        sigle = d.get("sigle")
        if sigle and sigle not in direction_map:
            direction_map[sigle] = d["id"]
        # Aussi mapper par code si c'est un sigle valide
        code = d.get("code")
...
def clean_directions():
    """Nettoie et consolide les directions"""
    print("\n🧹 Nettoyage des directions...")

    # Récupérer toutes les directions
    result = supabase.table("directions").select("*").execute()

    # Garder seulement les 9 directions principales
    # Désactiver les doublons (garder celles avec code = sigle)
    kept = set()
    for d in result.data:
        code = d.get("code")
        sigle = d.get("sigle")

        # Garder si le code est dans notre liste de directions officielles
        if code in DIRECTIONS:
            if code not in kept:
                kept.add(code)
                print(f"  ✅ Gardé: {code} - {d['label']}")
        elif sigle in DIRECTIONS and sigle not in kept:
            # Mettre à jour le code pour correspondre au sigle
            supabase.table("directions").update({"code": sigle}).eq("id", d["id"]).execute()
            kept.add(sigle)
            print(f"  ✅ Mis à jour: {sigle} - {d['label']}")

    print(f"\n  Total directions actives: {len(kept)}")
    return get_direction_ids()
...
def update_existing_profile(profile_id: str, data: dict):
    """Met à jour un profil existant"""
    supabase.table("profiles").update(data).eq("id", profile_id).execute()

def create_profile(user_data: dict, direction_id: str):
    """Crée un profil utilisateur dans la table profiles"""

    role_hierarchique = determine_role_hierarchique(user_data["poste"], user_data["categorie"])
    profil_fonctionnel = determine_profil_fonctionnel(user_data["poste"], user_data["direction"])

    profile_data = {
        "email": user_data["email"],
        "first_name": user_data["prenom"],
        "last_name": user_data["nom"],
        "full_name": f"{user_data['nom']} {user_data['prenom']}",
        "matricule": user_data["matricule"],
        "poste": user_data["poste"],
        "direction_id": direction_id,
        "direction_code": user_data["direction"],
...
def import_personnel():
    ...
    for i, agent in enumerate(PERSONNEL, 1):
        ...
        if direction not in direction_ids:
            print(f"  ❌ Direction {direction} non trouvée pour {agent['nom']} {agent['prenom']}")
            stats["errors"] += 1
            continue
        ...
        profile_data = create_profile(agent, direction_id)
        ...
        try:
            if existing:
                ...
                update_existing_profile(existing["id"], profile_data)
                print(f"  🔄 [{i:02d}/51] Mis à jour: {agent['nom']} {agent['prenom']} ({direction})")
                stats["updated"] += 1
            else:
                # Générer un nouvel ID pour le profil
                profile_data["id"] = str(uuid.uuid4())

                # Insérer le nouveau profil
                supabase.table("profiles").insert(profile_data).execute()
                print(f"  ✅ [{i:02d}/51] Créé: {agent['nom']} {agent['prenom']} ({direction})")
                stats["created"] += 1

        except Exception as e:
            print(f"  ❌ [{i:02d}/51] Erreur pour {agent['nom']}: {str(e)}")
            stats["errors"] += 1
...
def main():
    print("=" * 60)
    print("🚀 IMPORT DU PERSONNEL ARTI DANS SYGFP")
    print("=" * 60)

    # Étape 1: Nettoyer les directions
    direction_ids = clean_directions()

    # Étape 2: Importer le personnel
